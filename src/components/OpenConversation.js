    ...
    <div className="d-flex flex-column flex-grow-1">
          ...
          {selectedConversation.messages.map((message, index) => {
            ...
            return (
              <div ref={lastMessage ? setRef : null} key={index} className={`my-1 d-flex flex-column ${message.fromMe ? 'align-self-end' : ''}`}>
                <div className={`rounded px-2 py-1 ${message.from ? 'bg-primary text-white' : 'border'}`}>{message.text}</div>
      ...
      <Form onSubmit={handleSubmit}>
        <FormGroup className="m-2">
          ...
          {/* <InputGroup.Append> <Button type="submit">Send</Button></InputGroup.Append> */}
        </FormGroup>
